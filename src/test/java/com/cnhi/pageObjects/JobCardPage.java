...
//**************************************************************************************************************************************
	...
	public void SelectCreatedAppointment(String customerName, String device) throws Exception {
		...
		for (int i = 0; i < 25; i++) {
			try {
				...
				if (device.equalsIgnoreCase("Android")) {
					xPath = "(//android.widget.Button[contains(@text,'Appointment Prepared')][contains(@text,'"
							+ customerName + "')])[last()]";
				} else {
		...
		for (int i = 0; i < 20; i++) {
			try {
				...
				if (device.equalsIgnoreCase("Android")) {
					...
					xPath = "(//XCUIElementTypeLink[contains(@name,'Appointment Prepared')][contains(@name,'"
							+ customerName + "')])[last()]";
				}
				MobileElement ele = (MobileElement) driver.findElement(By.xpath(xPath));
	...
	public void SelectReceivedAppointment(String device) throws Exception {
		...
		for (int i = 0; i < 20; i++) {
			try {
				...
				String currentDate = gMob.getCurrentDate("MM/dd/yyyy");
				if (device.equalsIgnoreCase("Android")) {
					xPath = "(//android.widget.Button[contains(@text,'Received " + currentDate + "')])[last()]";
				} else {
					xPath = "(//XCUIElementTypeLink[@name='Received']/following-sibling::XCUIElementTypeLink[contains(@name,'"
							+ currentDate + "')])[last()]";
				}
				MobileElement ele = (MobileElement) driver.findElement(By.xpath(xPath));
	...
	public String getContactPhoneNumberNoFromJobDetail(String device) throws Exception {
		...
		if (device.equalsIgnoreCase("Android")) {
			contactPhoneNo = gMob.getElementText(contactPhoneNumberNoJobDetailTxtBox, "text",
					"Getting Value Contact Phone Number Text");
		} else {
			contactPhoneNo = gMob.getElementText(contactPhoneNumberNoJobDetailTxtBox, "value",
					"Getting Value Contact Phone Number Text");
		}
	...
	public String getRepairAddressFromJobDetail(String device) throws Exception {
		...
		if (device.equalsIgnoreCase("Android")) {
			repairAddress = gMob.getElementText(repairAddressJobDetailTxtBox, "text",
					"Getting Value Repair address Text");
		} else {
			repairAddress = gMob.getElementText(repairAddressJobDetailTxtBox, "value",
					"Getting Value Repair address Text");
		}
	...
	public String getCustomerComplainEmailFromJobDetail(String device) throws Exception {
		...
		if (device.equalsIgnoreCase("Android")) {
			customerComplain = gMob.getElementText(customerComplainsJobDetailTxtBox, "text",
					"Getting Value Customer Complain Text");
		} else {
			customerComplain = gMob.getElementText(customerComplainsJobDetailTxtBox, "value",
					"Getting Value Customer Complain Text");
		}
